...
if __name__ == '__main__':
    ...
    class MyDataset(torch.utils.data.Dataset):
        def __init__(self, split):
            ...
            split_idx = np.load(split_file, allow_pickle=True).item()
            idx = split_idx[split]
            ...
            for i in range(1, args.num_hops + 1):
                x = np.load(f'{dataset.dir}/process_shang/time_split/x_{split}_{i}.npy')
    ...
    for _, y in test_loader:
        y_test.append(y.to(torch.long))
    res['y_true'] = torch.cat(y_test, dim=0)
