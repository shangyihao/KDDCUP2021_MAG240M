...
if __name__ == '__main__':
    ...
    x_train = np.load(f'{dataset.dir}/process_shang/time_split/x_train_{args.layer}.npy')
    x_train = torch.from_numpy(x_train).to(device)
    x_valid = np.load(f'{dataset.dir}/process_shang/time_split/x_valid_{args.layer}.npy')
    x_valid = torch.from_numpy(x_valid).to(device)
    x_test = np.load(f'{dataset.dir}/process_shang/time_split/x_test_{args.layer}.npy')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if valid_acc > best_valid_acc:
            ...
            with torch.no_grad():
                model.eval()
                x_test1 = torch.from_numpy(x_test).to(device)
                res['y_pred'] = model(x_test1).argmax(dim=-1)
                del x_test1
        ...
        if epoch % 1 == 0:
            print(f'Epoch: {epoch:03d}, Loss: {loss:.4f}, '
                  ...
                  f', Best_valid: {best_valid_acc:.4f}')

    y_test = torch.from_numpy(dataset.paper_label[test_idx])
    y_test = y_test.to(device, torch.long)
    res['y_true'] = y_test
