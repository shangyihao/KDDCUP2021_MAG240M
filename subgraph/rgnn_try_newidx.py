...
class Temporal_emd(MessagePassing):
    ...
    def __init__(self):
        ...
        self.att = torch.nn.Parameter(torch.Tensor(1, 1))
        self.gap = None
    ...
    def forward(self, years, edge_index, x):
        ...
        # print(f' gap: {self.gap}, gap.shape: {self.gap.shape}')
        #
        # alpha = (self.att.to(device=x.device) * self.gap.to(device=x.device))
        # print(f'alpha: {alpha}, alpha.shape: {alpha.shape}')
        # print(f'x: {x}, x.shape: {x.shape}')

        out = self.propagate(edge_index, x=x, years=years)

        # print(f'out: {out}, out.shape: {out.shape}')
        return out

    def message(self, x_j,  years):
        year1 = years[0].float()
        year2 = years[1].float()
        self.gap = torch.abs(year1 - year2)
        alpha = (self.att.to(device=x_j.device) * self.gap.to(device=x_j.device)).sum(dim=-1)
        return x_j*alpha.unsqueeze(-1)

class Temporal_emd1(MessagePassing):

    def __init__(self):
        super(Temporal_emd1, self).__init__()
        self.att = torch.nn.Parameter(torch.Tensor(1, 1), requires_grad=False)
        self.gap = None

    def forward(self, years, edge_index, x):
        return self.propagate(edge_index, x=x, years=years)
...
class Temporal_emd2(MessagePassing):

    def __init__(self, in_channels):
        super(Temporal_emd2, self).__init__()
        self.att = torch.nn.Parameter(torch.Tensor(1, 1))
        self.gap = None
    ...
    def message(self, x_j, years):
        year1 = years[0].float()
        year2 = years[1].float()
        self.gap = torch.abs(year1 - year2).exp_().pow_(-1)
        print(f'gap.unsqueeze(-1).shape: {self.gap.unsqueeze(-1).shape}')
        print(f'x_j.shape: {x_j.shape}')
        alpha = (self.att.to(device=x_j.device) * self.gap.to(device=x_j.device))
        alpha = alpha.sum(dim=-1)

        return x_j * alpha.unsqueeze(-1)

class Temporal_emd3(MessagePassing):

    def __init__(self, in_channels):
        super(Temporal_emd3, self).__init__()
        self.in_channels = in_channels
        self.att = torch.nn.Parameter(torch.Tensor(1, self.in_channels))
        self.lin = Linear(1, self.in_channels)
    ...
    def message(self, x_j, years):
        year1 = years[0].float()
        year2 = years[1].float()
        gap = torch.abs(year1 - year2).unsqueeze(-1).to(device=x_j.device)
        gap = self.lin(gap).view(-1, self.in_channels)
        alpha = (gap.to(device=x_j.device) * self.att.to(device=x_j.device))
        # alpha = (self.att.to(device=x_j.device) * self.gap.to(device=x_j.device))
        alpha = alpha.sum(dim=-1)

        return x_j * alpha.unsqueeze(-1)

class Temporal_emd4(MessagePassing):

    def __init__(self, in_channels):
        super(Temporal_emd4, self).__init__()
        self.in_channels = in_channels
        self.att = torch.nn.Parameter(torch.Tensor(1, self.in_channels), requires_grad=False)
        self.lin = Linear(1, self.in_channels)
...
class Temporal_emd5(MessagePassing):

    def __init__(self, in_channels):
        super(Temporal_emd5, self).__init__()
        self.in_channels = in_channels
        self.att = torch.nn.Parameter(torch.Tensor(1, self.in_channels))
        self.lin = Linear(1, self.in_channels)
...
class Temporal_emd6(MessagePassing):

    def __init__(self, in_channels):
        super(Temporal_emd6, self).__init__()
        self.in_channels = in_channels
        self.att = torch.nn.Parameter(torch.Tensor(1, self.in_channels), requires_grad=False)
        self.lin = Linear(1, self.in_channels)
    ...
    def message(self, x_j, years):
        ...
        gap = torch.abs(year1 - year2).unsqueeze(-1).to(device=x_j.device)
        gap = self.lin(gap).view(-1, self.in_channels)
        alpha = (gap.to(device=x_j.device) * self.att.to(device=x_j.device)).sum(dim=-1)
        alpha = F.leaky_relu(alpha, self.negative_slope)
        alpha = F.dropout(alpha, p=self.dropout, training=self.training)
        return x_j * alpha.unsqueeze(-1)

class Temporal_emd7(MessagePassing):

    def __init__(self, in_channels):
        super(Temporal_emd7, self).__init__()
        self.in_channels = in_channels
        self.att = torch.nn.Parameter(torch.Tensor(1, self.in_channels), requires_grad=False)
        self.lin = Linear(1, self.in_channels)
        self.negative_slope = 0.2
...
class Temporal_emd8(MessagePassing):

    def __init__(self, in_channels):
        super(Temporal_emd8, self).__init__()
        self.in_channels = in_channels
        self.att = torch.nn.Parameter(torch.Tensor(1, self.in_channels))
...
class Temporal_emd_draw(MessagePassing):

    def __init__(self, in_channels):
        super(Temporal_emd_draw, self).__init__()
        self.att = torch.nn.Parameter(torch.Tensor(1, 1))
        self.gap = None
        self._alpha = None
    ...
    def message(self, x_j, years):
        ...
        # self.gap = torch.abs(year1 - year2)
        # gap = self.gap
        gap = torch.abs(year1 - year2).exp_().pow_(-1)
        # print(f'gap.unsqueeze(-1).shape: {self.gap.unsqueeze(-1).shape}')
        # print(f'x_j.shape: {x_j.shape}')
        alpha = (self.att.to(device=x_j.device) * gap.to(device=x_j.device))
        # alpha = alpha.sum(dim=-1)
        self._alpha = alpha.view(-1)

        return x_j * alpha.unsqueeze(-1)

class Temporal_emd_draw1(MessagePassing):

    def __init__(self, in_channels):
        super(Temporal_emd_draw1, self).__init__()
        self.att = torch.nn.Parameter(torch.Tensor(1, 1))
    ...
    def message(self, x_j, index: Tensor, ptr: OptTensor, years):
        year1 = years[0].float()
        year2 = years[1].float()
        # self.gap = torch.abs(year1 - year2)
        # gap = self.gap
        gap = torch.abs(year1 - year2)
        # print(f'gap.unsqueeze(-1).shape: {self.gap.unsqueeze(-1).shape}')
        # print(f'x_j.shape: {x_j.shape}')
        alpha = (self.att * gap.to(device=x_j.device)) + self.b
...
class Temporal_emd_1(MessagePassing):

    def __init__(self, in_channels):
        super(Temporal_emd_1, self).__init__()
        self.in_channels = in_channels
        self.a = torch.nn.Parameter(torch.Tensor(1, self.in_channels))
        self.b = torch.nn.Parameter(torch.Tensor(1, self.in_channels))
    ...
    def forward(self, years, edge_index, x):
        ...
        x1 = self.lin1(x).view(-1, self.in_channels)
        x2 = self.lin2(x).view(-1, self.in_channels)
        alpha_a = (x1 * self.a).unsqueeze(-1).sum(dim=-1)
        alpha_b = (x2 * self.b).unsqueeze(-1).sum(dim=-1)
        # b = (x * self.b).unsqueeze(-1).sum(dim=-1)

        return self.propagate(edge_index, x=x, alpha_a=alpha_a, years=years, alpha_b=alpha_b)


    def message(self, x_j, alpha_a_j, index, ptr, years, alpha_b_j):

        year1 = years[0].float()
        year2 = years[1].float()
        gap = torch.abs(year1 - year2).unsqueeze(-1)
        alpha = (alpha_a_j * gap.to(device=x_j.device)) + alpha_b_j
        alpha = F.leaky_relu(alpha, 0.2)
        # alpha = alpha.unsqueeze(-1)
        alpha = softmax(alpha, index, ptr)
        alpha = alpha.sum(dim=-1)
...
class Temporal_emd_2(MessagePassing):

    def __init__(self, in_channels):
        super(Temporal_emd_2, self).__init__()
        self.in_channels = in_channels
        self.a = torch.nn.Parameter(torch.Tensor(1, self.in_channels))
        self.b = torch.nn.Parameter(torch.Tensor(1, self.in_channels))
        self.c = torch.nn.Parameter(torch.Tensor(1, self.in_channels))
        self.lin1 = Linear(self.in_channels, self.in_channels)
        self.lin2 = Linear(self.in_channels, self.in_channels)
        self.lin3 = Linear(self.in_channels, self.in_channels)
        self.negative_slope = 0.2
        self.dropout = 0.0
        self.gap = None
        self._alpha = None
    ...
    def forward(self, years, edge_index, x):
        ...
        x1 = self.lin1(x).view(-1, self.in_channels)
        x2 = self.lin2(x).view(-1, self.in_channels)
        x3 = self.lin3(x).view(-1, self.in_channels)
        alpha_a = (x1 * self.a).unsqueeze(-1).sum(dim=-1)
        alpha_b = (x2 * self.b).unsqueeze(-1).sum(dim=-1)
        alpha_c = (x3 * self.b).unsqueeze(-1).sum(dim=-1)
        # b = (x * self.b).unsqueeze(-1).sum(dim=-1)

        return self.propagate(edge_index, x=x, alpha_a=alpha_a, years=years, alpha_b=alpha_b, alpha_c=alpha_c)


    def message(self, x_j, alpha_a_j, index, ptr, years, alpha_b_j, alpha_c_j):

        year1 = years[0].float()
        year2 = years[1].float()
        gap = torch.abs(year1 - year2).unsqueeze(-1)
        gap2 = torch.abs(year1 - year2).pow_(2).unsqueeze(-1)
        alpha = (alpha_a_j * gap2.to(device=x_j.device)) + (alpha_b_j * gap.to(device=x_j.device)) + alpha_c_j
        # alpha = (alpha_a_j * gap.to(device=x_j.device)) + alpha_b_j
        alpha = F.leaky_relu(alpha, 0.2)
        # alpha = alpha.unsqueeze(-1)
        # alpha = softmax(alpha, index, ptr)
...
class Temporal_emd_3(MessagePassing):
    def __init__(self, in_channels):
        super(Temporal_emd_3, self).__init__()
        self.in_channels = in_channels
    ...
    def message(self, x_j, alpha_j, alpha_i, index, ptr, years):
        year1 = years[0].float().to(device=x_j.device)
        year2 = years[1].float().to(device=x_j.device)

        alpha_j = alpha_j * year1.unsqueeze(-1)
        alpha_r = alpha_i * year2.unsqueeze(-1)
        alpha = alpha_j + alpha_r
        alpha = F.leaky_relu(alpha, 0.2)
        alpha = softmax(alpha, index, ptr)
        alpha = alpha.sum(dim=-1)
        alpha = F.dropout(alpha, p=0.5, training=self.training)
...
class Temporal_emd_4(MessagePassing):
    def __init__(self, in_channels):
        super(Temporal_emd_4, self).__init__()
        self.in_channels = in_channels
        self.att1 = torch.nn.Parameter(torch.Tensor(1, self.in_channels))
        self.att2 = torch.nn.Parameter(torch.Tensor(1, self.in_channels))
        self.lin1 = Linear(self.in_channels, self.in_channels)
    ...
    def message(self, x_j, alpha_j, alpha_i, index, ptr, years):
        ...
        alpha_j = alpha_j * year1.unsqueeze(-1)
        alpha_r = alpha_i * year2.unsqueeze(-1)
        alpha = torch.abs(alpha_j - alpha_r)
        alpha = F.leaky_relu(alpha, 0.2)
        alpha = softmax(alpha, index, ptr)
        alpha = alpha.sum(dim=-1)
        alpha = F.dropout(alpha, p=0.5, training=self.training)
...
class MAG240M(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        path_root = '/data/shangyihao/mag240m/new_feat(train_valid)0.1'

        # train_idx_path = osp.join(path_root, 'train_index.pt')
        # self.train_idx = torch.load(train_idx_path)
        self.train_idx = dataset.get_idx_split('train')
        self.train_idx = self.train_idx
        self.train_idx.share_memory_()
        # valid_idx_path = osp.join(path_root, 'valid_index.pt')
        # self.val_idx = torch.load(valid_idx_path)
    ...
    def val_dataloader(self):
        out = NeighborSampler(self.adj_t, node_idx=self.val_idx,
                               sizes=self.sizes, return_e_id=False,
                               transform=self.convert_batch,
...
class RGNN(LightningModule):
            ...
            for j in range(self.num_relations):
                ...
                if subadj_t.nnz() > 0:
                    if j == 0:
                        ...
                        out += self.convs[i][j]((x, x_target), subadj_t)
                # if j == 0 and subadj_t.nnz() > 0:
                #     row, col, _ = subadj_t.coo()
                #     # print(f'row.shape:{row.shape}')
                #     # print(f'col.shape: {col.shape}')
                #     year1 = years[row]
                #     year_target = years[col]
                #     out += self.emb[i]((year1, year_target), subadj_t, (x, x_target))
                #     # x_target = self.emb[i]((year1, year_target), subadj_t, (x, x_target))
                # else:
                #     out += self.convs[i][j]((x, x_target), subadj_t)
    ...
    def training_step(self, batch, batch_idx: int):
        # dataset = MAG240MDataset(2, 0.1, 0)
        # years = torch.from_numpy(dataset.all_paper_year)
...
if __name__ == '__main__':
    ...
    if not args.evaluate:
        ...
        trainer = Trainer(gpus=args.device, max_epochs=args.epochs,
                          ...
                          default_root_dir=f'logs/{args.model}_newidx')
        trainer.fit(model, datamodule=datamodule)
        # emb_layer = model.emb[1]
        #
        # def draw(x, y):
        #     x = x.numpy()
        #     y = y.cpu().detach().numpy()
        #     _dict = {}
        #     for year, _alpha in zip(x, y):
        #         if year not in _dict:
        #             _dict[year] = []
        #         _dict[year].append(_alpha)
        #     gap_years = []
        #     alphas = []
        #     keys = sorted(list(_dict.keys()))
        #     for gap_year in keys:
        #         value = _dict[gap_year]
        #         gap_years.append(gap_year)
        #         alphas.append(np.mean(value))
        #     plt.plot(gap_years, alphas)
        #     plt.xlabel('year gap')
        #     plt.ylabel('alpha')
        #     plt.show()
        #
        # year_gaps = emb_layer.gap
        # alpha_values = emb_layer._alpha
        # # att = emb_layer.att
        # # a = emb_layer.a
        # # b = emb_layer.b
        # # print(f'att: {att}')
        # # print(f'a: {a}')
        # # print(f'b: {b}')
        # draw(year_gaps, alpha_values)


    if args.evaluate:
        dirs = glob.glob(f'logs/{args.model}_newidx/lightning_logs/*')
        version = max([int(x.split(os.sep)[-1].split('_')[-1]) for x in dirs])
        logdir = f'logs/{args.model}_newidx/lightning_logs/version_{version}'
        ...
        for batch in tqdm(loader):
            ...
            with torch.no_grad():
                out = model(batch.x, batch.adjs_t, years).argmax(dim=-1).cpu()
                y_preds.append(out)
