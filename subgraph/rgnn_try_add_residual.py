...
class RGNN(LightningModule):
    def __init__(self, model: str, in_channels: int, out_channels: int,
        ...
        self.emb = ModuleList()
        self.first_lin = Linear(in_channels, hidden_channels)
    ...
    def forward(self, x: Tensor, adjs_t: List[SparseTensor], years) -> Tensor:
        ...
        #     print(adj_t)
        #     print(adj_t.size(0))
        x_first = x[:adjs_t[1].size(0)]
        x_first = F.relu(self.first_lin(x_first))
        x_first = F.dropout(x_first, p=self.dropout, training=self.training)
        for i, adj_t in enumerate(adjs_t):
            ...
            x = self.norms[i](out)
            x = x + self.fuse_weight[i] * x0
            x = F.elu(x) if self.model == 'rgat' else F.relu(x)
            x = F.dropout(x, p=self.dropout, training=self.training)
        x = x + self.fuse_weight[2] * x_first
        return self.mlp(x)
