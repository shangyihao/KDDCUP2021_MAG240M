...
class MAG240M(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        year_path = '/data/shangyihao/mag240m/RGNN/year_paper_author.npy'
...
class RGNN(LightningModule):
    def __init__(self, model: str, in_channels: int, out_channels: int,
            ...
            self.convs.append(
                ModuleList([
                    GATConv(in_channels, hidden_channels // heads, heads,
                            add_self_loops=False) for _ in range(num_relations - self.num_embs)
                ]))
            ...
            for _ in range(num_layers - 1):
                self.convs.append(
                    ModuleList([
                        ...
                        for _ in range(num_relations - self.num_embs)
                    ]))
        ...
        self.embs.append(
            ModuleList([
                Temporal_emd_6(in_channels, hidden_channels) for _ in range(self.num_embs)
            ]))
    ...
    def forward(self, x: Tensor, adjs_t: List[SparseTensor], years) -> Tensor:
        ...
        for i, adj_t in enumerate(adjs_t):
            ...
            for j in range(self.num_relations):
                ...
                #     x_target_temp = self.embs[i][j]((year1, year_target), subadj_t, (x, x_target))
                #     out += self.convs[i][j]((x, x_target_temp), subadj_t)
                if subadj_t.nnz() > 0:
                    if j < self.num_embs:
                        row, col, _ = subadj_t.coo()
                        year1 = years[row]
                        year_target = years[col]
                        out += self.embs[i][j]((year1, year_target), subadj_t, (x, x_target))
                        # out += self.convs[i][j]((x, x_target_temp), subadj_t)
                    else:
                        out += self.convs[i][4 - j]((x, x_target), subadj_t)
