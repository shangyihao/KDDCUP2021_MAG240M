...
class MAG240M(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        x = np.memmap(f'/data/shangyihao/mag240m/RGNN/full_feat0.1.npy', dtype=np.float16,
                           mode='r', shape=(N, self.num_features))
        self.x = torch.from_numpy(x)
        self.y = torch.from_numpy(dataset.all_paper_label)

        path = f'/data/shangyihao/mag240m/newedges0.1/full_adjt_same_author.pt'
...
class RGNN(LightningModule):
    ...
    #     train_loss_epoch = []
    #     for out in outputs:  # out is train_losses of all batches
    #         loss = np.mean(out)
