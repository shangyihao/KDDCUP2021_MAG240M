...
class RGNN(LightningModule):
    def __init__(self, model: str, in_channels: int, out_channels: int,
        ...
        self.embs.append(
            ModuleList([
                Temporal_emd_5(in_channels) for _ in range(num_relations)
            ]))

        for _ in range(num_layers - 1):
            self.embs.append(ModuleList([
                Temporal_emd_5(hidden_channels) for _ in range(num_relations)
            ]))

        # self.embs.append(Temporal_emd_7(in_channels, in_channels))
        # self.embs.append(Temporal_emd_7(hidden_channels, hidden_channels))
