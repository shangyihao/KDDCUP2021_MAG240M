...
class FeatCombine(torch.nn.Module):
    def __init__(self, prefer_channels, hidden_channels):
        super(FeatCombine, self).__init__()
        self.lin1 = Linear(prefer_channels, hidden_channels)
        self.att = torch.nn.Parameter(torch.Tensor(1, hidden_channels))

    def forward(self, seman_feat, prefer_feat):
        prefer = self.lin1(prefer_feat)
        alpha = (prefer * self.att).unsqueeze(-1).sum(dim=-1)
        x = alpha * seman_feat
        return x
...
class MAG240M(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        preference_x = np.memmap('/data/shangyihao/mag240m/new_feat(train_valid)0.1_0.7/type_5/feat_prefer.npy',
                                 dtype=np.float16, mode='c', shape=(N, (self.num_classes+1)))
        self.x = [semantic_x, preference_x]
    ...
    def convert_batch(self, batch_size, n_id, adjs):
        ...
        prefer_x = self.x[1][n_id.numpy()]
        seman_x = torch.from_numpy(seman_x).to(torch.float)
        prefer_x = torch.from_numpy(prefer_x).to(torch.float)
        x = [seman_x, prefer_x]
...
class RGNN(LightningModule):
    def __init__(self, in_channels: int, out_channels: int,
        ...
        self.preferenceConv = RGAT(154, 64, num_relations, num_layers, heads, dropout)
        self.concat = FeatCombine(64, hidden_channels)
    ...
    def forward(self, x, adjs_t: List[SparseTensor], years) -> Tensor:
        ...
        preference_feat = self.preferenceConv(prefer_x, adjs_t, years)

        # x = torch.cat([semantice_feat, preference_feat], dim=1)
        x = self.concat(semantice_feat, preference_feat)
        return self.mlp(x)
