...
parser = argparse.ArgumentParser(description='Training GNN on mag240m graph')

parser.add_argument('--data_dir', type=str, default=f'{dataset.dir}/process_shang/subgraph/2/OGB_MAG.pk',
                    help='The address of preprocessed graph.')
parser.add_argument('--model_dir', type=str, default=f'{dataset.dir}/process_shang/subgraph/2/hgt_4layer_notime',
...
for epoch in np.arange(args.n_epoch) + 1:
    ...
    for node_feature, node_type, edge_time, edge_index, edge_type, (train_mask, valid_mask, test_mask), ylabel in datas:
        ...
        train_acc  = evaluator.eval({
                        ...
                        'y_pred': train_res.view(-1).argmax(dim=1).unsqueeze(-1)
                    })['acc']
        valid_acc  = evaluator.eval({
                        'y_true': ylabel[valid_mask].unsqueeze(-1),
                        'y_pred': valid_res.view(-1).argmax(dim=1).unsqueeze(-1)
                    })['acc']
        # test_acc   = evaluator.eval({
        #                 'y_true': ylabel[test_mask].unsqueeze(-1),
        #                 'y_pred': test_res.argmax(dim=1).unsqueeze(-1)
        #             })['acc']
        stat += [[train_loss.item(), train_acc, valid_acc]]
    ...
    print('Epoch: %d  Train Loss: %.4f Train Acc: %.4f Valid Acc: %.4f ' % \
         (epoch,  avgs[0], avgs[1], avgs[2]))
    st = time.time()
