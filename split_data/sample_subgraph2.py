...
def create_map(ets):
    ...
    def add_map(_map, edge_type):
        ...
        if edge_type == 'cites':
            ...
            col, row = edge_index  # let institution as source node
            print(f'Done! [{time.perf_counter() - t:.2f}s]')
...
def sample_subgraph(_map, sampled_num=8, sampled_depth=2, inp=None):
    ...
    '''
    for layer in tqdm(range(sampled_depth)):
    ...
    def sampled_save():
        ...
        for nt in node_types:
            node_set = list(sample_data[nt])
            node_set.sort()
            print(f'num of node_{nt}: {len(node_set)}')
            print(f'save sampled node_{nt}...', end=' ', flush=True)
            t = time.perf_counter()
            path = f'{dataset.dir}/process_shang/subsample/subgraph2/node_{nt}.npy'
        ...
        for edge_type in edge_types:
            ...
            t = time.perf_counter()
            path = f'{dataset.dir}/process_shang/subsample/subgraph2/edge_index_{edge_type}.npy'
