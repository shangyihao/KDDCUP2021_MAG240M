...
if __name__ == '__main__':
    ...
    for j in range(0, num_features, 128):  # Run spmm in column-wise chunks...
        ...
        for i in range(1, args.num_layers + 1):
            x = adj_t @ x
            np.save(f'{dataset.dir}/process_shang/x_train_{i}_{j}.npy', x[train_idx].numpy())
            np.save(f'{dataset.dir}/process_shang/x_valid_{i}_{j}.npy', x[valid_idx].numpy())
            np.save(f'{dataset.dir}/process_shang/x_test_{i}_{j}.npy', x[test_idx].numpy())
            pbar.update(1)
    pbar.close()
    ...
    for i in range(1, args.num_layers + 1):
        ...
        for j in range(0, num_features, 128):
            x_train += [np.load(f'{dataset.dir}/process_shang/x_train_{i}_{j}.npy')]
            x_valid += [np.load(f'{dataset.dir}/process_shang/x_valid_{i}_{j}.npy')]
            x_test += [np.load(f'{dataset.dir}/process_shang/x_test_{i}_{j}.npy')]
        x_train = np.concatenate(x_train, axis=-1)
        x_valid = np.concatenate(x_valid, axis=-1)
        x_test = np.concatenate(x_test, axis=-1)
        np.save(f'{dataset.dir}/process_shang/x_train_{i}.npy', x_train)
        np.save(f'{dataset.dir}/process_shang/x_valid_{i}.npy', x_valid)
        np.save(f'{dataset.dir}/process_shang/x_test_{i}.npy', x_test)
    print(f'Done! [{time.perf_counter() - t:.2f}s]')
    ...
    for i in range(1, args.num_layers + 1):
        for j in range(0, num_features, 128):
            os.remove(f'{dataset.dir}/process_shang/x_train_{i}_{j}.npy')
            os.remove(f'{dataset.dir}/process_shang/x_valid_{i}_{j}.npy')
            os.remove(f'{dataset.dir}/process_shang/x_test_{i}_{j}.npy')
    print(f'Done! [{time.perf_counter() - t:.2f}s]')
