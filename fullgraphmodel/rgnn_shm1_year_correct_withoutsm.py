...
class Temporal_emd_6(MessagePassing):
    ...
    def forward(self, years, edge_index, x):
        ...
        alpha_l = (x1 * self.att1).sum(dim=-1).unsqueeze(-1)
        alpha_r = (x2 * self.att2).sum(dim=-1).unsqueeze(-1)
...
class RGNN(LightningModule):
    def __init__(self, model: str, in_channels: int, out_channels: int,
        ...
        self.emb.append(Temporal_emd_6(in_channels, in_channels))
        self.emb.append(Temporal_emd_6(hidden_channels, hidden_channels))
...
if __name__ == '__main__':
    ...
    if not args.evaluate:
        ...
        trainer = Trainer(gpus=args.device, max_epochs=args.epochs,
                          ...
                          default_root_dir=f'logs/{args.model}_full_withoutsm')
    ...
    if args.evaluate:
        ...
        evaluator.save_test_submission(res, f'results/{args.model}_withoutsm')
