...
class MAG240M(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        self.x = np.memmap(f'/dev/shm/full_feat.npy', dtype=np.float16,
                           mode='r', shape=(N, self.num_features))
        self.y = torch.from_numpy(dataset.all_paper_label)
        year_path = '/data/shangyihao/mag240m/RGNN/all_year_full.npy'
...
class RGNN(LightningModule):
    def __init__(self, model: str, in_channels: int, out_channels: int,
        ...
        self.embs.append(
            ModuleList([
                Temporal_emd_6(in_channels, in_channels) for _ in range(self.num_embs)
            ]))

        for _ in range(num_layers - 1):
            self.embs.append(ModuleList([
                Temporal_emd_6(hidden_channels, hidden_channels) for _ in range(self.num_embs)
            ]))
            ...
            for j in range(self.num_relations):
                ...
                if subadj_t.nnz() > 0:
                    if j < self.num_embs:
                        ...
                        year1 = years[row]
                        year_target = years[col]
                        x_target = self.embs[i][j]((year1, year_target), subadj_t, (x, x_target))
                        out += self.convs[i][j]((x, x_target), subadj_t)
                    else:
                        out += self.convs[i][j]((x, x_target), subadj_t)
...
if __name__ == '__main__':
    ...
    if args.evaluate:
        ...
        for batch in tqdm(loader):
            ...
            with torch.no_grad():
                out = model(batch.x, batch.adjs_t, batch.years).argmax(dim=-1).cpu()
                y_preds.append(out)
        res = {'y_pred': torch.cat(y_preds, dim=0)}
        evaluator.save_test_submission(res, f'results/{args.model}_year')
