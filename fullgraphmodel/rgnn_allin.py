...
class MAG240M(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        self.x = torch.from_numpy(np.memmap(f'{dataset.dir}/full_feat.npy', dtype=np.float16,
                           mode='r', shape=(N, self.num_features)))
    ...
    def train_dataloader(self):
        ...
        out_sample = NeighborSampler(self.adj_t, node_idx=self.train_idx,
                                     sizes=self.sizes, return_e_id=False,
                                     transform=self.convert_batch,
                                     batch_size=self.batch_size, shuffle=True,
                                     num_workers=8)
        print(f'train_sample time: {(time.perf_counter() - t_s):.3f} ')
        return out_sample

    def val_dataloader(self):
        t_s = time.perf_counter()
        out_sample = NeighborSampler(self.adj_t, node_idx=self.val_idx,
                               sizes=self.sizes, return_e_id=False,
                               transform=self.convert_batch,
                               batch_size=self.batch_size, num_workers=6)
        print(f'valid_sample time: {(time.perf_counter() - t_s):.3f} ')
        return out_sample
    ...
    def convert_batch(self, batch_size, n_id, adjs):
        ...
        print(f'load batch feature: {(time.perf_counter() - t_feat):.3f}')
        ...
        for i, adj_t in enumerate(adjs_t):
            ...
            for j in range(self.num_relations):
                ...
                if j == 0:
                    ...
                    x_target = self.emb[i]((year1, year_target), subadj_t, x)
                if subadj_t.nnz() > 0:
            ...
            result = self.mlp(x)
            print(f'process time: {(time.perf_counter() - t):.3f} ')
        return result

    def training_step(self, batch, batch_idx: int):
        dataset = MAG240MDataset(root=ROOT)
        years = torch.from_numpy(dataset.all_paper_year)
        y_hat = self(batch.x, batch.adjs_t, years)
...
if __name__ == '__main__':
    ...
    if not args.evaluate:
        ...
        trainer = Trainer(gpus=args.device, max_epochs=args.epochs,
                          ...
                          default_root_dir=f'logs/{args.model}_full')
        trainer.fit(model, datamodule=datamodule)

    if args.evaluate:
        dirs = glob.glob(f'logs/{args.model}_full/lightning_logs/*')
        version = max([int(x.split(os.sep)[-1].split('_')[-1]) for x in dirs])
        logdir = f'logs/{args.model}_full/lightning_logs/version_{version}'
