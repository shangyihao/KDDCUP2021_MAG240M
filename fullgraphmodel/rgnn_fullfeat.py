...
class MAG240M(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        feat_path = osp.join(path_root, 'full_feat.npy')
        self.x = np.memmap(feat_path, dtype=np.float16,
                           mode='c', shape=(N, (dataset.num_classes + dataset.num_paper_features)))
        # self.x = torch.from_numpy(x)
        self.y = torch.from_numpy(dataset.all_paper_label)

        path = f'/data/shangyihao/mag240m/RGNN/full_adj_t0.1.pt'
...
class RGNN(LightningModule):
    def __init__(self, model: str, in_channels: int, out_channels: int,
        ...
        super().__init__()
        self.dataset = MAG240MDataset(2, 0.1, 0)
